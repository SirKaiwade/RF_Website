...
set library_role = 'edit'
where library_role is null;

alter table public.profiles
  alter column library_role set default 'edit';
...
create policy "banned_emails all" on public.banned_emails
...
create policy "library_folder_viewers all" on public.library_folder_viewers
  for all using (true) with check (true);
...
create policy "library_folder_locks all" on public.library_folder_locks
  for all using (true) with check (true);
